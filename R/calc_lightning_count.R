################################################################################
# title: Calculate Lightning Count
...
dat_sf <- dat_sf |> 
  select(-1) |> 
  mutate(no = 1:780019) |> 
  relocate(no, .before = "geometry")

# convert to dt and name the geometry column as geometry
dat_dt <- data.table(st_drop_geometry(dat_sf[, "no"]),
                     as_geos_geometry(dat_sf))
...
for (i in 2016:2018) {
  ...
  write_sf(
    dat_sf, paste0(
      "./data/output/calc_lightning_count/lg_grd", 
      "_year_", 
      i, 
      ".gpkg"
    )
